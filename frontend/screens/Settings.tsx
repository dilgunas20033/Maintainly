...
export default function Settings({ navigation }: any) {
  ...
  return (
    ...
    <View style={s.wrap}>
      ...
      <Pressable onPress={save} style={[s.btn, saving && { opacity: 0.6 }]} disabled={saving}><Text style={s.btnText}>{saving ? 'Saving…' : 'Save'}</Text></Pressable>

      <Text style={[s.section, { color: colors.text }]}>Manage</Text>
      <Pressable onPress={() => navigation.navigate('HomesManager')} style={s.linkBtn}><Text style={s.linkText}>Homes</Text></Pressable>
      <Pressable onPress={() => navigation.navigate('AppliancesList', { homeId: (profile as any)?.last_home_id })} style={s.linkBtn}><Text style={s.linkText}>Appliances</Text></Pressable>
      <Pressable onPress={() => navigation.navigate('Calendar', { homeId: (profile as any)?.last_home_id })} style={s.linkBtn}><Text style={s.linkText}>Calendar</Text></Pressable>
